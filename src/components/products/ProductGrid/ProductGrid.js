...
const INTENTS = {
  waitlist: {
    ...
    sub: 'Be first through the doors when the shala, temple and ice spa open beside the café.',
    fields: ['name', 'email'],
    submit: 'Join the waitlist',
    doneH: "You're on the list.",
    doneP: "We'll write to you the moment doors open.",
  },
  notify: {
    flag: 'Opening 2026',
    ...
    submit: 'Notify me',
    doneH: 'Noted.',
    doneP: 'A single note will reach you the day the Sanctuary opens.',
  },
  host: {
    flag: 'Private & group bookings',
    title: 'Host at the Sanctuary',
    sub: 'Workshops, retreats, sound circles or private sessions — tell us what you have in mind.',
    fields: ['name', 'email', 'message'],
    submit: 'Send enquiry',
    doneH: 'Thank you.',
    doneP: "Your enquiry is with our team — we'll reply within two working days.",
  },
};
...
function SanctuaryModal({ intent, onClose }) {
  ...
  React.useEffect(() => {
    if (intent) { setVals({}); setDone(false); }
  }, [intent]);

  React.useEffect(() => {
    const esc = (e) => { if (e.key === 'Escape') onClose(); };
    if (intent) document.addEventListener('keydown', esc);
      ...
      <div className="sa-panel" role="dialog" aria-modal="true">
        ...
        {done ? (
          <div className="sa-done">
            <div className="sa-tick">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M5 12l5 5L19 7" strokeLinecap="round" strokeLinejoin="round" /></svg>
            </div>
            <h3>{cfg.doneH}</h3>
            <p className="sa-sub">{cfg.doneP}</p>
            <button className="btn sa-submit" style={{ maxWidth: 220, margin: '0 auto' }} onClick={onClose}>Close</button>
          </div>
        ) : (
          <>
            <div className="sa-flag"><span className="dot" />{cfg.flag}</div>
            <h3>{cfg.title}</h3>
            ...
            <form className="sa-form" onSubmit={handleSubmit}>
              {cfg.fields.includes('name') && (
                <label className="sa-field">
                  <span>Your name</span>
                  <input type="text" required value={vals.name || ''} onChange={e => setVals(v => ({ ...v, name: e.target.value }))} placeholder="First & last" />
                </label>
              )}
              {cfg.fields.includes('email') && (
                <label className="sa-field">
                  <span>Email</span>
                  <input type="email" required value={vals.email || ''} onChange={e => setVals(v => ({ ...v, email: e.target.value }))} placeholder="you@email.com" />
                </label>
              )}
              {cfg.fields.includes('message') && (
                <label className="sa-field sa-wide">
                  <span>Tell us about your gathering <em>(optional)</em></span>
                  <textarea value={vals.message || ''} onChange={e => setVals(v => ({ ...v, message: e.target.value }))} placeholder="Type, dates, headcount…" />
                </label>
              )}
              <button type="submit" className="btn forest sa-submit">{cfg.submit} <Arrow /></button>
              <p className="sa-fine">We will never share your details with third parties.</p>
            </form>
          </>
        )}
      </div>
